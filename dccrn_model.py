...
class DCCRN(nn.Module):
    ...
    def __init__(
                    ...
                    rnn_layers=2,
                    rnn_units=128,
                    win_len=1200,      # Increased from 400 (25ms at 48kHz)
                    win_inc=300,       # Increased from 100 (6.25ms at 48kHz)
                    fft_len=1536,      # Increased from 512 
                    win_type='hann',
                    masking_mode='E',
                    use_clstm=False,
                    use_cbn = False,
        ...
        ''' 
            Modified for 48kHz audio processing
            
            win_len: 1200 samples = 25ms at 48kHz (was 400 samples = 25ms at 16kHz)
            win_inc: 300 samples = 6.25ms at 48kHz (was 100 samples = 6.25ms at 16kHz)
            fft_len: 1536 (was 512) - increased for better frequency resolution
...
if __name__ == '__main__':
    ...
    # Updated for 48kHz: 4 seconds at 48kHz = 192,000 samples
    inputs = torch.randn([10, 48000*4]).clamp_(-1,1)
    labels = torch.randn([10, 48000*4]).clamp_(-1,1)
